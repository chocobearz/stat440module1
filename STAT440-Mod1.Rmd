...
train[,11:40] <- (train[,11:40])*1
test[,11:40] <- (test[,11:40])*1
>>>>>>> ce38097f994763fa1c716b05856c1a315d0b36b4
```
...
model2 <- randomForest(temp_duration ~ ., data = completeTrain,ntree = 400, mtry = 6, importance = TRUE)
model2
```
...
model3 <- randomForest(duration ~ respiratory+confirmed+province+V1+age, data = completeTrain, ntree = 200, mtry = 6, importance = TRUE)
model3
```
...
predictions_train <- predict(model3, completeTrain)
eval_results(y_train, predictions_train, completeTrain) 

# Make predictions on test set
predictions_test <- predict(model3, test)
```
